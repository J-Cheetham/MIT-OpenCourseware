...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for h in range(0, height):
            for w in range(0, width):
                tile = (w, h)
                self.tile_dict.update({tile: dirt_amount})
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        x = pos.get_x()
        y = pos.get_y()
        current_tile = (math.floor(x), math.floor(y))

        for tile in self.tile_dict:
            if tile == current_tile:
                dirt = self.tile_dict.get(tile)
                if dirt - capacity < 0:
                    self.tile_dict[tile] = 0
                else:
                    self.tile_dict[tile] = (dirt - capacity)
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        dirt_list = self.tile_dict.values()
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        #Make sure robot doesn't spawn outside the room or under furniture
        while True:
            #choose random number between 0 and width/height
            x = random.uniform(0, self.room.get_width())
            y = random.uniform(0, self.room.get_height())
            self.position = Position(x, y)
            if self.room.is_position_valid(self.position):
                break
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        return len(self.tile_dict) - len(self.furniture_tiles)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_position = self.position.get_new_position(angle, speed)
        if self.room.is_position_valid(new_position):
            ...
            self.set_robot_direction(new_direction) #TODO using setters from within the class. Necessary?
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        #robots get starting position initialized upon creation so they must be re-initialized at beginning of each trial
        robots = []
        for i in range(0, num_robots):
            robots.append(robot_type(room, speed, capacity))
        coverage = 0
        time_steps = 0
        #anim = ps3_visualize.RobotVisualization(1, 5, 5, []) # TODO what are this?
        ...
        #anim.done()
        time_step_results.append(time_steps)
    #return the average time step required to clean room
    return math.fsum(time_step_results) / len(time_step_results)
